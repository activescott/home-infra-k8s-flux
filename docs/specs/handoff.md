...
- Renovate follow-ups (setup complete 2026-09-03; open loose ends in summary "Remaining / future": Micah's wp-admin DB-upgrade click after WP 7.0.4, grafana sidecar hot-reload 401, k3s upgrade then raise kubectl cap, gpu-poet Dependabot triage) — [summary](renovate-setup/summary.md)
...
- Age-key-only secrets (#159): [plan](age-key-only-secrets/plan.md) approved; tooling (`show`/`edit`/`new`/`rotate-age-key`/`migrate --verify`, age key read from 1Password) is in. Left to do is Scott's migration run (verify, encrypt the two orphans, delete the attachments) and then a second PR removing `push`/`pull`, `migrate`, and the `.public-key-encrypted` suffix.
