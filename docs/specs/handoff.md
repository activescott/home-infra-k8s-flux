...
- opencode ACP (activeassistant#42): the acpx `stateDir` fix is deployed but unexercised, since `/state/openclaw/acpx-state` is only created on the first ACP spawn — verify with a real opencode session before closing — [summary](olya-readonly-instructions/summary.md)
