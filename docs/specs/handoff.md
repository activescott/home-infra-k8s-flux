...
- 1Password backup of repo plaintext secrets: `scripts/onepassword-secrets.mts` is written and verified end-to-end, but only the `transmission` group has actually been pushed — the other 55 files / ~30 items still need a full `push` — [summary](onepassword-secret-backup/summary.md)
