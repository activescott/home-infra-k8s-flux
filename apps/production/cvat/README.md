...
The upstream [CVAT Helm chart](https://docs.cvat.ai/docs/administration/advanced/k8s_deployment_with_helm/)
...
**Perf watch:** the chart's `permissionFix` init container runs `chmod -R 777` over the
data volume on every backend pod start. Fine while small; if pod starts get slow once
the chunk cache grows, set `cvat.backend.permissionFix.enabled: false` in `helm.yaml`
and ensure `data/`, `keys/`, `logs/` under the PV path are owned by uid 1000
(the same lesson as photoprism's `PHOTOPRISM_DISABLE_CHOWN`).
